...
- **bootseed-web**：Nginx 提供 iPXE 启动脚本、Alpine 启动文件与镜像下载（支持 Range）。
...
```text
 ...
 │                                                                │
 │   ┌────────────────────┐        ┌──────────────────────────┐  │
 │   │   bootseed-pxe       │        │       bootseed-web        │  │
 │   │ dnsmasq ProxyDHCP    │        │ Nginx (HTTP, Range 支持)   │  │
 │   │ + TFTP (host network)│        │  /boot/boot.ipxe          │  │
 │   │ UDP 67 / 69 / 4011   │  反代 / │  /alpine/<arch>/...       │  │
 │   │ Option93 架构路由     │ 与 /api │  /images/index.json+raw   │  │
 │   └─────────┬───────────┘        └─────────┬──────┬──────────┘  │
 │             │                              │      │ 反代          │
 │             │                     静态下载   │      ▼             │
 │             │                              ▼  ┌───────────────┐  │
 │             │                       (大文件/Range) │ bootseed-server │  │
 │             │                              │  │ 门户后端(Go)      │  │
 │             │                              │  │ 镜像增删/节点登记 │  │
 │             │                              │  │ bbolt 持久化     │  │
 │             │                              │  └───────┬───────┘  │
...
服务端为三个容器(docker-compose):`bootseed-pxe`(ProxyDHCP+TFTP)、
`bootseed-web`(Nginx,静态下载 + 反代)、`bootseed-server`(门户后端 + bbolt 持久化)。
对外只有 Nginx 一个地址与端口;`/`、`/api/*` 反代到 bootseed-server,
`/boot`、`/alpine`、`/images` 为静态文件(支持 Range,大镜像可断点续传)。
...
| TCP `HTTP_PORT`    | HTTP：boot.ipxe / Alpine 启动文件 / 镜像   | bootseed-web (nginx)   |
...
- **进入 iPXE 后卡住 / 下载失败**：确认 `bootseed-web` 正常、`HTTP_PORT` 可达、`PXE_SERVER_IP` 正确。
