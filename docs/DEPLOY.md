...
`bootseed-agent` → 浏览器访问节点 `http://<节点IP>:<AGENT_PORT>` 选镜像写盘。
...
- Linux 宿主机，与目标节点处于同一二层广播域（或已配置 DHCP Relay，见第 10 节）。
...
```
data/tftp/x86/undionly.kpxe           # BIOS PXE
data/tftp/x86_64/snponly.efi          # UEFI x86_64
...
1. 在目标机固件中**关闭 Secure Boot**（见第 11 节），选择一次性网络启动（PXE）。
2. 确保目标机与 BootSeed 宿主机**在同一二层**（或已配置 DHCP Relay，见第 10 节）。
3. 启动后流程：
