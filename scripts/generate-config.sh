...
kernel \${deploy_server}/alpine/${arch}/vmlinuz ${console} modules=loop,squashfs,sd-mod,usb-storage modloop=\${deploy_server}/alpine/${arch}/modloop deploy_server=\${deploy_server} node_arch=\${node_arch} node_mac=\${node_mac} node_uuid=\${node_uuid} agent_port=\${agent_port} bootseed_origin=pxe network_device_timeout=${NETWORK_DEVICE_TIMEOUT} storage_device_timeout=${STORAGE_DEVICE_TIMEOUT}
